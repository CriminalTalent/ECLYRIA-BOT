  ...
  def self.parse(mastodon_client, sheet_manager, mention)
    begin
      ...
      when /\[구매\/(.+?)\]/
        ...
        puts "[DEBUG] 구매 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[양도\/(.+?)\/@(.+?)\]/
        ...
        puts "[DEBUG] 아이템 양도 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[양도\/갈레온\/(\d+)\/@(.+?)\]/
        ...
        puts "[DEBUG] 갈레온 양도 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[사용\/(.+?)\]/
        ...
        puts "[DEBUG] 아이템 사용 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[주머니\]/
        ...
        puts "[DEBUG] 주머니 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[타로\]/
        ...
        puts "[DEBUG] 타로 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[베팅\/(\d+)\]/
        ...
        puts "[DEBUG] 베팅 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[(\d+)D\]/
        ...
        puts "[DEBUG] 주사위 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[동전\]/
        ...
        puts "[DEBUG] 동전 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
      ...
      when /\[YN\]/i
        ...
        puts "[DEBUG] YN 명령 결과: #{message.inspect}"
        if message
          result = mastodon_client.reply(mention.status, message)
          puts "[DEBUG] 답글 전송 완료: #{result.id}"
        end
