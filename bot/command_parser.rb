...
module CommandParser
  ...
  def self.handle_betting(mention, acct, display_name, bet_amount)
    ...
    if result_multiplier == 0
      ...
      result_message = "베팅 금액이 그대로 돌아왔습니다.\n잔여 갈레온: #{user_info['galleons']}G\n오늘 남은 베팅: #{remaining_bets}회"
  ...
  def self.handle_help(mention, acct, display_name)
    help_text = <<~HELP
      ...
      🎮 미니게임:
       [베팅/금액] - 도박 시스템 (1-20G, 하루 3회)
         └ 결과: x(-5~+5), 빚 가능성 있음 ⚠️
       [20D] - 20면 주사위 굴리기
       [100D] - 100면 주사위 굴리기  
       [yn] - YES/NO 점술
       [운세] - 오늘의 타로카드 (해석+행운색+행운물건)
       [동전] - 동전 던지기
      
       ⚠️ 베팅으로 빚이 생기면 갚기 전까지 구매 불가!
       🎓 신규 입학은 교수봇에서 [입학/이름]으로 하세요!
    HELP
  ...
  def self.handle_unknown(mention, acct, display_name, text)
    unknown_responses = [
      ...
      "#{display_name}님, 상점 이용이나 미니게임을 즐겨보세요! '도움말' 참고!"
    ]
